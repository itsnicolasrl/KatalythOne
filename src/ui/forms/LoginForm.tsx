...
export function LoginForm() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setShowRecover(data?.code === "ACCOUNT_PENDING_DELETION");
        return;
      }
      router.push(next);
    } catch {
  ...
  async function onRecoverAccount() {
    setRecovering(true);
    setError(null);
    try {
      ...
      setError(data?.message ?? "Cuenta recuperada. Ahora inicia sesión.");
  ...
  return (
    <div className="w-full h-full bg-white rounded-3xl shadow-2xl overflow-y-auto flex flex-col items-center justify-center py-14 px-8">
      ...
      {/* Zona de inputs: ancho limitado, centrado */}
      <div className="flex flex-col gap-3 w-[78%]">
        ...
        {showRecover ? (
          <button
            ...
            className="w-full rounded-full border border-[#F28705]/40 bg-[#fff7ef] hover:bg-[#ffefdf] transition-all py-3 text-sm font-bold text-[#c85f03] disabled:opacity-60"
          >
            {recovering ? "Recuperando..." : "Recuperar cuenta (30 días)"}
          </button>
        ) : null}
        ...
        <button
          onClick={onSubmit}
          disabled={loading}
          className="w-full rounded-full bg-[#F28705] hover:bg-[#F27405] active:scale-[0.98] transition-all py-3.5 text-sm font-bold text-white shadow-lg shadow-[#F28705]/25 disabled:opacity-60 mt-1"
        >
